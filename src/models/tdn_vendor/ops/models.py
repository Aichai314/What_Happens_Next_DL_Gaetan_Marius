...
class TSN(nn.Module):
    ...
    def _prepare_base_model(self, base_model, num_segments):
        ...
        if 'resnet' in base_model :
            # LOCAL EDIT (TDN integration, Marius): pretrained=False. The upstream
            # default fetches a long-dead lip6.fr ImageNet URL; our SSv2 checkpoint
            # overwrites every weight anyway, so this only skips a broken download.
            # Module structure / state_dict keys are unchanged.
            self.base_model = tdn_net(base_model, num_segments, pretrained=False)
