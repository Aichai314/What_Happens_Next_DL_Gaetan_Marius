...
def inject_tsm_into_coatnet(model: nn.Module, num_frames: int, n_div: int = 8) -> nn.Module:
    """
    Injects TSM into the early MBConv stages of a timm CoAtNet.
    CoAtNet stages 0 and 1 are convolutional. Stages 2 and 3 are Attention-based.
    """
    print(f"--> Injecting TSM into CoAtNet early convolutional stages...")
    if hasattr(model, 'stages'):
        # Target only the first two stages (the convolutional MBConv blocks)
        for i in range(min(2, len(model.stages))): 
...
def inject_dropout_into_coatnet(model: nn.Module, attn_drop_rate: float, proj_drop_rate: float) -> nn.Module:
    """
    Dynamically crawls through the instantiated model and updates the .p 
    attribute of nn.Dropout layers inside attention and MLP blocks.
    """
    print(f"--> Injecting attention dropout ({attn_drop_rate}) and projection dropout ({proj_drop_rate})...")
    
    attn_drop_count = 0
    proj_drop_count = 0
    
    for name, module in model.named_modules():
        # Update standard Attention Dropout
        if hasattr(module, 'attn_drop') and isinstance(module.attn_drop, nn.Dropout):
            module.attn_drop.p = attn_drop_rate
            attn_drop_count += 1
            
        # Update standard Projection Dropout
        if hasattr(module, 'proj_drop') and isinstance(module.proj_drop, nn.Dropout):
            module.proj_drop.p = proj_drop_rate
            proj_drop_count += 1
            
        # Catch standard MLP dropouts in timm (often just named 'drop' inside mlp blocks)
        if 'mlp' in name.lower() and hasattr(module, 'drop') and isinstance(module.drop, nn.Dropout):
            module.drop.p = proj_drop_rate
            proj_drop_count += 1

    print(f"    - Updated {attn_drop_count} attention dropout layers.")
    print(f"    - Updated {proj_drop_count} projection/MLP dropout layers.")
...
@two_stage_trainer
class CoAtNet_Hybrid(nn.Module):
    """
    The Ultimate Kaggle Hybrid Model: 
    CoAtNet (Conv+Attention Spatial) + TSM (Early Temporal) + Custom Head (Late Temporal)
    """
    def __init__(self,
        ...
        self.attn_drop_rate = model_cfg.get('attention_dropout', 0.1)
        self.proj_drop_rate = model_cfg.get('projection_dropout', 0.1)
        
        # 1. Load CoAtNet Backbone (without classifier)
        backbone_name = model_cfg.get('backbone', 'coatnet_0_rw_224')
        ...
        # Determine feature dimension (e.g., 768 for coatnet_0)
        self.in_features = self.backbone.num_features
        
        # 2. Inject TSM into the Convolutional Stages
        if model_cfg.get('use_tsm', True):
            self.backbone = inject_tsm_into_coatnet(
                self.backbone, 
                num_frames=self.num_frames, 
                n_div=model_cfg.get('fold_div', 8)
            )
        self.backbone = inject_dropout_into_coatnet(
            self.backbone, 
        ...
        if self.head_type == 'linear':
            ...
            self.lstm = nn.LSTM(
                ...
                num_layers=1, 
                batch_first=True,
                bidirectional=True  # Bidirectional doubles the output feature size back to in_features
            )
            self.head = nn.Sequential(
                nn.Dropout(self.dropout_rate),
    ...
    def get_param_groups(
        ...
        # Include LSTM parameters if using LSTM head
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.head_type in ['linear', 'mlp']:
            ...
            # LSTM inherently processes the sequence
            ...
            # Attention head manages its own sequence processing and pooling
            out = self.head(features)
            
        return out

# --- Quick Test Code (Uncomment to verify) ---
